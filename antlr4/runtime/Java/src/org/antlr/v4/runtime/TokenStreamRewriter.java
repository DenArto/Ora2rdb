...
public class TokenStreamRewriter {
	...
	public String getText(String programName, Interval interval) {
		...
		Map<Integer, RewriteOperation> indexToOp = reduceToSingleOperationPerIndex(rewrites, interval);
	 ...
	 */
	protected Map<Integer, RewriteOperation> reduceToSingleOperationPerIndex(List<RewriteOperation> rewrites, Interval interval) {
//		System.out.println("rewrites="+rewrites);
		int start = interval.a;
		int stop = interval.b;
		ArrayList<Integer> intervalOps = new ArrayList<Integer>();

		for (int i = 0; i < rewrites.size(); i++) {
			RewriteOperation op = rewrites.get(i);

			if (op != null && op.index >= start && op.index <= stop)
				intervalOps.add(i);
		}

		// WALK REPLACES
		for (int i = 0; i < intervalOps.size(); i++) {
			Integer opIdx = intervalOps.get(i);
			RewriteOperation op = rewrites.get(opIdx);
			if ( op==null ) continue;
			if ( !(op instanceof ReplaceOp) ) continue;
			ReplaceOp rop = (ReplaceOp)op;
			// Wipe prior inserts within range
			List<Integer> inserts = getKindOfOps(rewrites, InsertBeforeOp.class, intervalOps, i);
			for (Integer iopIdx : inserts) {
				InsertBeforeOp iop = (InsertBeforeOp)rewrites.get(iopIdx);
				if ( iop.index == rop.index ) {
					// E.g., insert before 2, delete 2..2; update replace
					// text to include insert before, kill insert
					rewrites.set(iopIdx, null);
			...
			List<Integer> prevReplaces = getKindOfOps(rewrites, ReplaceOp.class, intervalOps, i);
			for (Integer prevRopIdx : prevReplaces) {
				ReplaceOp prevRop = (ReplaceOp)rewrites.get(prevRopIdx);
				if ( prevRop.index>=rop.index && prevRop.lastIndex <= rop.lastIndex ) {
					// delete replace as it's a no-op.
					rewrites.set(prevRopIdx, null);
					continue;
				}
				...
				if ( prevRop.text==null && rop.text==null && !disjoint ) {
					...
					rewrites.set(prevRopIdx, null); // kill first delete
		...
		for (int i = 0; i < intervalOps.size(); i++) {
			...
			InsertBeforeOp iop = (InsertBeforeOp)op;
			// combine current insert with prior if any at same index
			List<Integer> prevInserts = getKindOfOps(rewrites, InsertBeforeOp.class, intervalOps, i);
			for (Integer prevIopIdx : prevInserts) {
				InsertBeforeOp prevIop = (InsertBeforeOp)rewrites.get(prevIopIdx);
				if ( prevIop.index==iop.index ) {
					if ( InsertAfterOp.class.isInstance(prevIop) ) {
						iop.text = catOpText(prevIop.text, iop.text);
						rewrites.set(prevIopIdx, null);
					}
			...
			List<Integer> prevReplaces = getKindOfOps(rewrites, ReplaceOp.class, intervalOps, i);
			for (Integer ropIdx : prevReplaces) {
				ReplaceOp rop = (ReplaceOp)rewrites.get(ropIdx);
				if ( iop.index == rop.index ) {
					rop.text = catOpText(iop.text,rop.text);
					rewrites.set(opIdx, null);	// delete current insert
					continue;
				}
	...
	protected <T extends RewriteOperation> List<Integer> getKindOfOps(List<? extends RewriteOperation> rewrites, Class<T> kind, List<Integer> ops, int before) {
		List<Integer> beforeOps = new ArrayList<Integer>();
		for (int i=0; i<before && i<ops.size(); i++) {
			Integer opIdx = ops.get(i);
			RewriteOperation op = rewrites.get(opIdx);
