...
sql_script
    : ( sql_plus_command_wrapper | unit_statement_wrapper )* EOF
    ;

unit_statement_wrapper
...
create_procedure_body
    : CREATE (OR REPLACE)? editioning_clause? PROCEDURE procedure_name ('(' parameter (',' parameter)* ')')?
...
create_trigger
    : CREATE ( OR REPLACE )? editioning_clause? TRIGGER trigger_name
...
identified_by
    : IDENTIFIED BY id_expression ( HTTP? DIGEST (ENABLE | DISABLE ) )?
    ;
...
column_definition
    : column_name (datatype | type_name)
         SORT?  (DEFAULT expression)? (ENCRYPT (USING  CHAR_STRING)?
...
tableview_name
    : identifier (PERIOD id_expression)?
          (AT_SIGN link_name ('.' link_name)? | /*TODO{!(input.LA(2) == BY)}?*/ partition_extension_clause)?
...
general_element_part
    ...
    ('.' id_expression)* ('@' link_name)? function_argument?
    ;
